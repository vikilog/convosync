...
export function parseInboxScope(value: unknown): InboxScope | null {
  ...
  const channels = Array.isArray(record.channels)
    ? record.channels.filter(
        (c): c is InboxChannel => typeof c === 'string' && INBOX_CHANNELS.includes(c as InboxChannel)
      )
    : undefined
...
export function allowedAccountIds(channel: InboxChannel, scope: InboxScope): string[] | 'all' | null {
  if (scope.mode === 'all') return 'all'
  return restrictedChannelAccess(scope).get(channel) ?? null
}

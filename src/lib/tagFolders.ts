/** Shared folder-normalization + grouping helpers for the WorkspaceTag registry (mirrors backend). */
...
/** Flat, folder-clustered tag names — used for `<datalist>` suggestions in free-text tag inputs. */
export function sortTagNamesByFolder(items: { name: string; folder: string | null }[]): string[] {
  return groupTagsByFolder(items).flatMap((g) => g.items.map((i) => i.name));
}

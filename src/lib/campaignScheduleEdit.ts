...
export const DELETE_SEND_LOCK_MS = 2 * 60 * 1000
...
export function canDeleteCampaign(
  ...
  if (s === 'scheduled' && scheduledAt) {
    return new Date(scheduledAt).getTime() - now >= DELETE_SEND_LOCK_MS
  }
  return true
}
...
export function wizardSeedFromCampaignDetail(detail: {
  ...
  const audienceType =
    detail.audienceType === 'all' || detail.segmentIds.some((id) => id === 'all') ? 'all' : 'segment'
  return {
    channel: detail.channel === 'email' || detail.channel === 'instagram' ? detail.channel : 'whatsapp',
    audienceType,
    ...
    replyHandling:
      detail.replyHandling === 'journey' || detail.replyHandling === 'ai_agent'
...
export function defaultScheduleLocal(fromMs = Date.now() + 60 * 60 * 1000): { date: string; time: string } {

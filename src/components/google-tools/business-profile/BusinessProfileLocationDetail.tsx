...
export function BusinessProfileLocationDetail({
  ...
  if (empty) {
    return (
      <section className="flex-1 min-w-0 flex flex-col items-center justify-center bg-slate-50 p-8">
        <div className="max-w-sm text-center rounded-2xl border border-swiss-line bg-white px-8 py-10 shadow-[0_4px_24px_rgba(52,168,83,0.06)]">
          ...
          <h3 className="text-base font-semibold text-gray-950">Select a location</h3>
  ...
  return (
    <section className="flex-1 min-w-0 flex flex-col bg-slate-50 overflow-hidden">
      <div className="sticky top-0 z-10 shrink-0 flex items-center gap-2 px-4 py-2.5 border-b border-swiss-line bg-white/95 backdrop-blur-md">
        {showBack && onBack && (
          <button
            type="button"
            onClick={onBack}
            className="lg:hidden p-2 rounded-lg text-swiss-muted hover:bg-slate-50"
      ...
      <div className="flex-1 min-h-0 overflow-y-auto overflow-x-hidden">
        <div className="max-w-[800px] mx-auto px-4 sm:px-6 py-6 sm:py-8 space-y-4">
          <article className="rounded-2xl border border-swiss-line bg-white shadow-[0_4px_24px_rgba(15,23,42,0.04)] overflow-hidden">
            <header className="px-6 py-5 border-b border-[#f0eef5]">
              <div className="flex items-start gap-4">
                ...
                <div className="min-w-0 flex-1">
                  <h1 className="text-xl sm:text-2xl font-semibold text-gray-950 leading-tight break-words">
                    {location.title || 'Untitled location'}
                  </h1>
                  {location.metadata?.placeId && (
                    <p className="text-meta text-swiss-faint mt-1 font-mono truncate">
            ...
            <div className="px-6 py-5 grid gap-5 sm:grid-cols-2">
              <div className="rounded-xl border border-swiss-line bg-slate-50 p-4">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <h2 className="text-sm font-bold uppercase tracking-wider text-swiss-muted">Address</h2>
                </div>
                <p className="text-sm text-swiss-ink leading-relaxed whitespace-pre-line">
              ...
              <div className="rounded-xl border border-swiss-line bg-slate-50 p-4">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <h2 className="text-sm font-bold uppercase tracking-wider text-swiss-muted">
                    Business hours
                  </h2>
                </div>
                {hours.length === 0 ? (
                  <p className="text-sm text-swiss-muted">No hours listed for this location.</p>
                ) : (
                  <dl className="space-y-1.5">
                    {hours.map((row) => (
                      <div key={row.day} className="flex justify-between gap-3 text-sm">
                        <dt className="font-semibold text-swiss-ink shrink-0">{row.label}</dt>
                        <dd className="text-swiss-muted text-right">{row.hours}</dd>
                      </div>
                    ))}
                  </dl>
                )}
          ...
          <article className="rounded-2xl border border-swiss-line bg-white shadow-[0_4px_24px_rgba(15,23,42,0.04)] overflow-hidden">
            <header className="px-6 py-4 border-b border-[#f0eef5] flex items-center gap-2">
              ...
              <h2 className="text-sm font-bold text-swiss-ink">Metrics</h2>
              <button
                type="button"
            ...
            <div className="px-6 py-5">
              {metricsLoading ? (
                <div className="flex items-center gap-2 text-sm text-swiss-faint">
                ...
                <dl className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    <dt className="text-swiss-muted text-sm font-bold uppercase">Total reviews</dt>
                    <dd className="text-lg font-black text-swiss-ink mt-1">
                      {String(reviewSummary.value.totalReviews ?? 0)}
                    </dd>
                  </div>
                  <div>
                    <dt className="text-swiss-muted text-sm font-bold uppercase">Avg rating</dt>
                    <dd className="text-lg font-black text-swiss-ink mt-1">
          ...
          <article className="rounded-2xl border border-swiss-line bg-white shadow-[0_4px_24px_rgba(15,23,42,0.04)] overflow-hidden">
            <header className="px-6 py-4 border-b border-[#f0eef5] flex items-center gap-2">
              ...
              <h2 className="text-sm font-bold text-swiss-ink">Reviews</h2>
              <span className="ml-auto text-meta text-swiss-faint tabular-nums">
                {reviews.length} cached
              </span>
            </header>
            <div className="px-6 py-4">
              {reviewsLoading ? (
                <div className="flex items-center justify-center py-8 text-sm text-swiss-faint">
                ...
                <div className="py-8 text-center">
                  ...
                  <p className="text-sm font-semibold text-swiss-muted">No cached reviews yet</p>
                  <p className="text-xs text-swiss-faint mt-1 max-w-sm mx-auto leading-relaxed">
                ...
                <ul className="divide-y divide-[#f0eef5]">
                  {reviews.map((review) => (
                    <li key={review.id} className="py-4 first:pt-0">
                      ...
                      {review.comment && (
                        <p className="text-sm text-swiss-muted mt-2 leading-relaxed">{review.comment}</p>
                      )}
                      {review.reviewReply && (
                        <p className="text-xs text-swiss-muted mt-2 pl-3 border-l-2 border-[#34A853]/30">

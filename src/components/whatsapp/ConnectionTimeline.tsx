...
export const ConnectionTimeline: FC<ConnectionTimelineProps> = ({ title, steps }) => {
  return (
        ...
        {steps.map((item, index) => {
          ...
          return (
            <li key={item.step} className="relative flex gap-5 pb-10 last:pb-0">
              {!isLast && (
                <span
                  className="absolute left-[18px] top-10 bottom-0 w-0.5 bg-gradient-to-b from-[#25D366]/50 to-slate-200"
                  aria-hidden
                />
              )}
              <span className="relative z-10 flex shrink-0 items-center justify-center w-9 h-9 rounded-full bg-[#25D366] text-white text-sm font-black shadow-md shadow-[#25D366]/25">
                {item.step}
              </span>

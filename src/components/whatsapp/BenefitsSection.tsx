      ...
      <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {items.map((item) => {
          ...
          return (
            <article
              key={item.id}
              className="p-5 rounded-xl border border-swiss-line bg-gradient-to-br from-white to-slate-50 hover:border-primary/20 transition-all"
            >
              <div className="w-11 h-11 rounded-xl bg-sky-50 text-primary flex items-center justify-center mb-4">
                <Icon className="w-5 h-5" strokeWidth={2} />
              </div>
              <h4 className="text-sm font-semibold text-swiss-ink">{item.title}</h4>
              <p className="mt-1.5 text-xs text-swiss-muted font-medium leading-relaxed">
                {item.description}
              </p>
            </article>
          );

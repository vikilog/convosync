...
export function AiProviderPanel() {
  const { data: config, isLoading } = realIntegrationsService.useAiProviderConfig()
  ...
  const draft = (): AiProviderUpdateInput => ({
    mode,
    ...(mode === 'byok'
      ? {
          provider,
      ...
      <CardContent className="space-y-4 pt-6">
        ...
        {mode === 'convosync' ? (
          <p className="text-muted-foreground text-sm">
            Managed AI is included with your plan — no key setup required.
          </p>
        ) : (
          <>
            <div className="space-y-1.5">

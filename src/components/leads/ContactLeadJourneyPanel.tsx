  ...
  return (
    <article className="rounded-xl border border-slate-200 bg-slate-50/80 p-3">
      <div className="flex items-start justify-between gap-2">
        ...
        <span className="rounded-md bg-white px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-wide text-gray-400 ring-1 ring-slate-200">
          Read-only
        </span>
      </div>
      ...
      <div className="mt-3 space-y-2">
        <p className="text-sm font-bold text-gray-900">{journey.funnelName}</p>
        ...
        {journey.origin?.commentText ? (
          <p className="rounded-lg border border-[#E1306C]/15 bg-[#fce8f0]/40 px-2.5 py-2 text-xs text-gray-700">
            <span className="font-bold text-[#C13584]">@{journey.origin.username}</span>
            {': '}
            {journey.origin.commentText}
          </p>
        ) : null}
        <ol className="relative ml-1.5 mt-3 space-y-3 border-l-2 border-slate-200 pl-4">
          {journey.timeline.map((item, i) => (
            <li key={`${item.at}-${i}`} className="relative">
              ...
              <p className="mt-0.5 text-xs font-semibold text-gray-900">{item.text}</p>

    ...
    >
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-1.5 min-w-0">
          ...
          {display ? (
            <span className={`truncate ${DISPLAY_TEXT_CLASS[field.type] || 'text-sm text-gray-900'}`}>
      ...
      {!display && (
        <p className="mt-1.5 text-xs text-gray-400 bg-slate-50 border border-slate-100 rounded px-2 py-1">
      ...
      {field.options.length > 0 && (
        <div className="mt-1.5 flex flex-wrap gap-1">
          {field.options.map((opt, i) => (
            <span
              key={i}
              className="text-[11px] px-2 py-0.5 rounded-full border border-slate-200 bg-slate-50 text-gray-500"
            >
              {opt}
            </span>
          ))}
    ...
    <div className="flex items-center gap-1.5 flex-wrap mb-3">
        ...
        <div key={s.id} className="relative group">
          ...
          {!readOnly && value.screens.length > 1 && (
            <button
              ...
              className="absolute -top-1.5 -right-1.5 w-4 h-4 rounded-full bg-white border border-slate-300 text-gray-400 hover:text-red-500 hover:border-red-300 items-center justify-center hidden group-hover:flex"
            >
      ...
      {!readOnly && (
        <button
          type="button"
          onClick={addScreen}
          className="px-3 py-1.5 rounded-lg text-xs font-bold border border-dashed border-slate-300 text-gray-500 hover:border-primary/40 hover:text-primary flex items-center gap-1"
        >
  ...
  return (
    <div className="flex flex-col h-full min-h-[560px]">
      ...
      <div className="grid grid-cols-[210px_minmax(0,1fr)_260px] gap-4 flex-1 min-h-0">
        <div className="bg-white ring-1 ring-slate-200/80 rounded-xl p-3 overflow-y-auto">
          {PALETTE.map((group) => (
            <div key={group.group} className="mb-4">
              <p className="text-[11px] font-bold uppercase tracking-wide text-gray-400 px-1.5 pb-2">
                {group.group}
              </p>
              {group.items.map((item) => {
                ...
                return (
                  <button
                    ...
                    className="w-full flex items-center gap-2 px-2.5 py-2 rounded-lg text-sm font-semibold text-gray-800 hover:bg-slate-50 text-left"
                  >
        ...
        <div className="rounded-xl p-6 overflow-y-auto flex flex-col items-center" style={CANVAS_BG_STYLE}>
          <p className="text-xs text-[#54656f] bg-white/90 px-2.5 py-1 rounded-md shadow-sm mb-4">
            Click a field on the left to add it, drag cards to reorder
          </p>
          <div className="w-full max-w-[340px] bg-white rounded-lg shadow-md overflow-hidden">
            <div className="px-3.5 pt-3 pb-2 border-b border-[#e9edef]">
              <input
                type="text"
                ...
                className="w-full font-semibold text-base text-[#111b21] outline-none bg-transparent"
              />
            </div>

            <div className="p-3 flex flex-col gap-2 min-h-[120px]">
              {activeScreen.fields.length === 0 ? (
                <div className="border border-dashed border-gray-300 rounded-lg py-8 px-4 text-center text-xs text-gray-400">
            ...
            <div className="px-3 pb-3 space-y-2">
              <input
                ...
                className="w-full text-center bg-primary text-white text-sm font-bold rounded-lg py-2.5 outline-none"
              />
              <div className="flex items-center gap-1.5">
                ...
                <select
                  ...
                  className="flex-1 bg-slate-50 border border-slate-200 rounded-lg py-1.5 px-2 text-xs outline-none focus:ring-2 focus:ring-primary/20"
                >
        ...
        <div className="bg-white ring-1 ring-slate-200/80 rounded-xl p-4 overflow-y-auto">
          ...
          {!selectedField ? (
            ...
            <div className="space-y-4">
              <label className="block">
                ...
                {selectedField.type === 'RichText' ? (
                  <textarea
                    ...
                    rows={4}
                    className="mt-1 w-full bg-slate-50 border border-slate-200 rounded-lg py-2 px-2.5 text-sm outline-none focus:ring-2 focus:ring-primary/20"
                  />
              ...
              {isMediaType(selectedField.type) && (
                <div>
                  ...
                  <label className="mt-1.5 flex items-center justify-center gap-1.5 border border-dashed border-slate-300 rounded-lg py-3 text-xs font-semibold text-gray-500 hover:border-primary/40 hover:bg-primary/5 cursor-pointer">
            ...
            <div className="space-y-4">
              ...
              <div>
                <span className="text-xs font-bold text-gray-500">Answer is saved as</span>
                <p className="mt-1 font-mono text-xs text-gray-500 bg-slate-50 border border-slate-200 rounded px-2 py-1.5 inline-block">
                  {selectedField.name}
                </p>
              ...
              {selectedField.type === 'TextInput' && (
                <label className="block">
                  <span className="text-xs font-bold text-gray-500">Answer type</span>
                  <select
              ...
              {selectedField.type === 'CalendarPicker' && (
                <label className="block">
                  <span className="text-xs font-bold text-gray-500">Selection</span>
                  <select
              ...
              <label className="flex items-center justify-between cursor-pointer">
                <span className="text-xs font-bold text-gray-500">Required to continue</span>
                <button
                  type="button"
              ...
              {isChoiceType(selectedField.type) && (
                <div>
                  <span className="text-xs font-bold text-gray-500">Choices</span>

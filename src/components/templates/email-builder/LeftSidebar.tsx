...
function BlockPaletteItem({
  ...
  return (
    <div
      ...
      className="flex items-start gap-3 p-3 bg-white border border-swiss-line hover:border-primary/40 cursor-grab active:cursor-grabbing transition-all"
    >
      ...
      <div className="min-w-0">
        <p className="text-sm font-semibold text-swiss-ink">{label}</p>
        <p className="text-xs text-swiss-muted truncate">{description}</p>
      </div>
    </div>
  );
}
...
export function LeftSidebar() {
  ...
  return (
    <aside className="w-[300px] shrink-0 border-r border-swiss-line bg-white flex flex-col min-h-0">
      <div className="grid grid-cols-5 border-b border-swiss-line bg-white">
        {TABS.map((tab) => (
          <button
            key={tab.id}
            type="button"
            ...
            className={`min-w-0 py-2 px-0.5 flex flex-col items-center justify-center gap-1 text-[10px] font-semibold leading-tight ${
              leftTab === tab.id
                ? 'text-primary border-b-2 border-primary bg-primary/[0.06]'
                : 'text-swiss-muted hover:text-swiss-ink border-b-2 border-transparent hover:bg-surface-muted'
            }`}
          >
            {tab.icon}
      ...
      <div className="flex-1 overflow-y-auto p-3 bg-surface-muted/40">
        {leftTab === 'blocks' && (
          <div className="space-y-2">
            <p className="text-xs text-swiss-muted px-1 mb-2">Drag blocks onto the canvas</p>
          ...
          <div className="space-y-3">
            {TEMPLATE_GALLERY.map((t) => (
              <button
                ...
                className="w-full text-left p-3 bg-white border border-swiss-line hover:border-primary/40 transition-all"
              >
                <p className="text-sm font-semibold text-swiss-ink">{t.name}</p>
                <p className="text-xs text-swiss-muted mt-0.5">{t.description}</p>
        ...
        {leftTab === 'sections' && (
          <div className="space-y-4">
            <div className="p-3 rounded-xl border border-dashed border-[#dadde1] bg-white">
              <p className="text-sm font-semibold text-swiss-ink mb-2">Save reusable section</p>
              <Input
                className="h-auto w-full mb-2 rounded-lg border border-swiss-line px-2 py-1.5 text-sm"
            ...
            <div>
              <p className="text-meta font-bold uppercase text-swiss-faint mb-2">Built-in</p>
              <div className="space-y-2">
                {BUILTIN_SECTIONS.map((s) => (
                  <button
                    ...
                    className="w-full text-left p-3 rounded-xl border border-swiss-line bg-white hover:border-primary/30 text-sm font-medium text-swiss-ink"
                  >
                    {s.name}
                  </button>
                ))}
            ...
            {sections.length > 0 && (
              <div>
                <p className="text-meta font-bold uppercase text-swiss-faint mb-2">Your sections</p>
                <div className="space-y-2">
                  {sections.map((s) => (
                    <div
                      key={s.id}
                      className="flex items-center gap-2 p-2 rounded-xl border border-swiss-line bg-white"
                    >
                      <button
                        type="button"
                        onClick={() => insertBlocks(s.blocks)}
                        className="flex-1 text-left text-sm font-medium text-swiss-ink truncate"
                      >
                        {s.name}
                      </button>
                      <button
                        ...
                        className="p-1 text-swiss-faint hover:text-red-500"
        ...
        {leftTab === 'brand' && (
          <div className="space-y-3">
            <label className="block space-y-1">
              <span className="text-sm font-semibold text-swiss-muted">Company name</span>
              <Input
            ...
            <label className="block space-y-1">
              <span className="text-sm font-semibold text-swiss-muted">Logo URL</span>
              <Input
                className="h-auto w-full rounded-lg border border-swiss-line px-3 py-2 text-sm"
                value={brand.logoUrl}
            ...
            <label className="block space-y-1">
              <span className="text-sm font-semibold text-swiss-muted">Primary color</span>
              <Input
                type="color"
                className="h-auto h-10 w-full rounded border border-swiss-line"
                value={brand.primaryColor}
                onChange={(e) => setBrand({ primaryColor: e.target.value })}
              />
            </label>
            <label className="block space-y-1">
              <span className="text-sm font-semibold text-swiss-muted">Text color</span>
              <Input
                type="color"
                className="h-auto h-10 w-full rounded border border-swiss-line"
            ...
            <label className="block space-y-1">
              <span className="text-sm font-semibold text-swiss-muted">Email background</span>
              <Input
                type="color"
                className="h-auto h-10 w-full rounded border border-swiss-line"

...
/** Runs when the user returns to a kept-alive panel (inactive → active). */
  ...
  return (
    <KeepAliveActiveContext.Provider value={active}>
      <KeepAliveActivationContext.Provider value={activation}>
        <div className={active ? 'flex h-full min-h-0 min-w-0 flex-1 flex-col' : 'hidden'} aria-hidden={!active}>
          {children}
        </div>

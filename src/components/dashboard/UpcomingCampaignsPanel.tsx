...
function channelIconClass(channel: QuickCampaign['channel']) {
  if (channel === 'whatsapp') return 'bg-emerald-50 text-emerald-600';
  if (channel === 'instagram') return 'bg-pink-50 text-pink-600';
  return 'bg-sky-50 text-sky-600';
}

function ChannelIcon({ channel }: { channel: QuickCampaign['channel'] }) {
  const cls = `h-4 w-4 ${channel === 'whatsapp' ? 'text-emerald-600' : channel === 'instagram' ? 'text-pink-600' : 'text-sky-600'}`;
...
export const UpcomingCampaignsPanel: React.FC<UpcomingCampaignsPanelProps> = ({
  ...
  <section className="rounded-xl bg-white p-5 ring-1 ring-slate-200/80">
    <div className="mb-4 flex items-center justify-between">
      <div>
        <h2 className="text-base font-semibold text-slate-900">Upcoming campaigns</h2>
        <p className="text-sm text-slate-500">Scheduled and draft campaigns ready to send</p>
      </div>
      <button
        type="button"
        onClick={onViewAll}
    ...
    {campaigns.length === 0 ? (
      <div className="rounded-lg border border-dashed border-slate-200 py-10 text-center">
        ...
        <p className="mt-3 text-sm font-medium text-slate-800">No upcoming campaigns</p>
        <p className="mt-1 text-sm text-slate-500">Schedule a campaign or save one as draft.</p>
        <button
          type="button"
          onClick={onNewCampaign}
          className="mt-4 inline-flex cursor-pointer items-center gap-1.5 text-sm font-medium text-sky-600 hover:text-sky-700"
        >
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-3">
        {campaigns.map((campaign) => (
          <button
            ...
            className="flex cursor-pointer flex-col rounded-xl border border-slate-200 bg-slate-50/40 p-4 text-left transition-colors hover:border-sky-200 hover:bg-white"
          >
            ...
            <p className="mt-3 line-clamp-2 text-sm font-semibold text-slate-900">{campaign.name}</p>

            <p className="mt-1 text-xs text-slate-500">{channelLabel(campaign.channel)}</p>

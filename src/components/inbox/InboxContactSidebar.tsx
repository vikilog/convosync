...
export const InboxContactSidebar: React.FC<Props> = ({
  ...
  return (
    <section className="flex h-full w-full shrink-0 flex-col bg-surface-muted text-left">
      {onClose && (
        <div className="flex items-center justify-between border-b border-black/5 bg-surface px-3 py-2.5">
          <p className="text-sm font-bold text-gray-900">Contact & journey</p>
          <button
            type="button"
            onClick={onClose}
          ...
          return (
            <button
              ...
              className={`flex flex-1 items-center justify-center gap-1.5 rounded-t-lg px-2 py-2 text-[11px] font-bold transition-colors duration-200 cursor-pointer ${
                active
                  ? 'bg-surface-muted text-primary border border-b-0 border-black/5'
